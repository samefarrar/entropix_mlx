...
def fit_dirichlet(
    ...
    for _ in range(max_iters):
        ...
        error_norm = mx.linalg.norm(error, axis = -1)
        ...
        new_alpha = mx.where(
            rearrange(new_converged, "v -> v 1"), alpha, mx.max(alpha + scaled_delta_alpha, alpha / 2)
        )
        alpha = new_alpha
...
tol: float = 1e-6,
    ...
    for _ in range(max_iters):
        ...
        new_converged = converged | (mx.abs(error) < tol)
        ...
        new_T = mx.where(new_converged, T, mx.max(T - delta_T, T / 2))
